...
export default class AddFood extends React.Component {
  ...
  goFirstPage = () => {
    API.doFetch(
      `/food/search?search=${this.state.lockedInSearchTerm}&pageNumber=${1}`
    ).then(res => {
      res = JSON.parse(res);
      if (res) {
  ...
  goLastPage = () => {
    let lastPage = Math.ceil(
      this.state.foodsFromSearch.totalHits / this.state.resultsPerPage
    );
    API.doFetch(
      `/food/search?search=${this.state.lockedInSearchTerm}&pageNumber=${lastPage}`
    ).then(res => {
      res = JSON.parse(res);
      if (res) {
        return this.setState({
  ...
  goNextPage = () => {
    if (
      ...
      API.doFetch(
        `/food/search?search=${this.state.lockedInSearchTerm}&pageNumber=${this
          .state.page + 1}`
      ).then(res => {
        res = JSON.parse(res);
        if (res) {
  ...
  handleFoodSubmit = e => {
    ...
    if (this.state.searchTerm) {
      API.doFetch(
        `/food/search?search=${this.state.searchTerm}&brand=${this.state.brandTerm}`
      ).then(res => {
        res = JSON.parse(res);
        if (res.foods) {
          return this.setState({
  ...
  render() {
    return (
      <div className="AddFoodDiv">
        <label htmlFor="searchTermInput">Search for a food:</label>
        <br></br>
        ...
        {this.state.displaySearchResults &&
          (this.state.gotNoResults ? (
            <div>No results found, try again or fix the search term</div>
          ) : (
            <div className="AddFoodSearchResults">
              <div>
                Only at most {this.state.resultsPerPage} items shown at once,
                total of {this.state.foodsFromSearch.totalHits}
              </div>
              ...
              {this.state.foodsFromSearch.foods.map((food, index) => (
                <div key={index} className="AddFoodSearchResultsDiv">
                  <div
                    className="AddFoodSearchResultsRow"
                    onClick={e => {
                      this.handleAddFood(e, food);
                    }}
                  >
              ...
              <div className="AddFoodSearchResultsPaginate">
                {this.state.page !== 1 && (
                  <button form="sub-form2" id="back" onClick={this.goPrevPage}>
                    Back
                  </button>
                )}
                <span>{this.state.foodsFromSearch.currentPage}</span>
                <button form="sub-form3" id="next" onClick={this.goNextPage}>
                  Next
                </button>
              </div>
            </div>
